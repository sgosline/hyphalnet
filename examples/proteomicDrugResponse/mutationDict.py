...
def tumor_genes(data_frame, group, subgroup, value):
    ...
    beta=0.5
    key = 'proteomics'
    this_hyp = hyphalNetwork(mutation_dictionary, g.copy())
    hyphae[key] = this_hyp
...
def main():
    ...
    if args.fromFile is None:
        hyphae = tumor_genes(data, 'AML sample', 'Gene', 'Tumor VAF')
    else:
    ...
    if args.getDist:
        ...
        res.to_csv('proteomicTumorDistances.csv')
        nmi = hyStats.compute_all_nmi(hyphae, g)
        nmi.to_csv('proteomictumor.csv')
